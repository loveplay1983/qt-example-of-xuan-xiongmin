...
void MainWindow::init()
{
    ...
    this->setWindowTitle(tr("病区标签扫描"));                       // window title
...
void MainWindow::keyPressEvent(QKeyEvent *keyValue)
{
    ...
    else
    {
        ...
        insertLabelItem.prepare("INSERT INTO wardScan(label, operator, eventTime, scanMark)"
...
void MainWindow::displayCurrentInput()
{
    ...
    header << tr("标签号") << tr("操作员") << tr("记录时间") << tr("扫描标记");
    ...
    QSqlQuery getScanMark(db);                                                 // customized scanMark
    ...
    getScanMark.prepare("SELECT scanMark FROM wardScan WHERE label = :currentLabel");
    getScanMark.bindValue(":currentLabel", this->label);
    getScanMark.exec();
    getScanMark.next();
    if(getScanMark.value(0).toInt() == 1)
    {
...
void MainWindow::delLineTempTable()
{
    ...
    if(index != -1)
    {
        ...
        queryDelLabel.prepare("DELETE FROM wardScan WHERE label = :label");
