    ...
    , ui(new Ui::MainWindow)
{
    ...
    // delete current line of data(temp input)
...
{
    int tempInputIndex = ui->currentInputTable->currentRow();
    if(tempInputIndex != -1)
    {
        // delete specified data in database table
        labelDel = ui->currentInputTable->item(tempInputIndex, 0)->text();
        QSqlQuery query;
        query.prepare("DELETE FROM lablabel WHERE label = :label");
        query.bindValue(":label", labelDel);
        if (!query.exec()){ QMessageBox::information(this, "提示", "删除条目出错" + query.lastError().text()); }
        else { QMessageBox::information(this, "提示", "所选条目" + labelDel + "已删除"); }

        // delete current row of qtablewdiget
        ui->currentInputTable->removeRow(tempInputIndex);

        ui->inputEntry->clear();       // clear the input entry
    }
}
